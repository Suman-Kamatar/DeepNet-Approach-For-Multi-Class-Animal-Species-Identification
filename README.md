...
- **Intelligent Classification**: Categorizes detections into three tiers:
    ...
    - 🟡 **Wild & Safe**: Zebra, Giraffe, Deer, Monkeys, etc.
    - 🔴 **Wild & Dangerous**: Lions, Tigers, Bears, Elephants, etc. (Triggers immediate alerts).
- **Multi-Channel Alerts**:
    - 🔊 **Audio**: High-decibel alert sound played locally (`alert.wav`).
...
- **Optimized Performance**: Frame skipping and inference resizing for smooth real-time operation.
...
2. Install dependencies:
   ```bash
   pip install opencv-python ultralytics flask flask-socketio pygame numpy
   ```
...
- **Performance**: Adjust `FRAME_SKIP` and `DETECTION_WIDTH` for your hardware.
- **Alerts**: Set `NOTIFICATION_METHOD` to 'EMAIL', 'SMS', or 'BOTH'.

---
*Developed for wild animal conservation and human safety.*
